      ...
      <ul className="legal-list">
        {legalIndex.map((doc) => (
          <li key={doc.id} className="legal-item">
            ...
            <a href={doc.path} target="_blank" rel="noopener noreferrer" className="legal-link">
              View Document
            </a>
            {doc.requiredAcceptance && (
              <div className="legal-acceptance">
                <span className="legal-accept-label">Acceptance:</span>{' '}
                {acceptance[doc.id] ? '✅ Accepted' : '❌ Not Accepted'}
    ...
    <div className="sovereign-interaction-history">
      ...
      <section>
        ...
        <ul>
          {remixes.map((consent, i) => (
            <li key={i}>
              {consent.timestamp}: Consent Hash {consent.consentHash}
            </li>
          ))}
        </ul>
      </section>
      <section>
        ...
        <ul>
          {comments.map((ack, i) => (
            <li key={i}>
              {ack.timestamp}: Comment {ack.commentId}
            </li>
          ))}
        </ul>
      </section>
