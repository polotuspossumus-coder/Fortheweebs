  ...
  async handleStripeWebhook(rawBody: Buffer, signature: string) {
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        const metadata = session.metadata as { subscriberId?: string; creatorId?: string; tier?: string } | null;
        if (!metadata?.subscriberId || !metadata?.creatorId || !metadata?.tier) {
          throw new Error('Missing required metadata in checkout session');
        }
        const { subscriberId, creatorId, tier } = metadata;
