  ...
  const verifyInfluencer = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (followerCount >= selectedPlatform.minFollowers * 2) {
          setTimeout(() => {
            setResult({
              success: true,
              message: `🎉 Congratulations! You've been verified as an influencer. All $500 CREATOR tier features are now FREE for you!`
            });
            if (onVerified) {
