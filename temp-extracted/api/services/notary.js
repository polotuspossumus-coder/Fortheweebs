...
function notaryRecord(event) {
    ...
    if (event.command && event.command.includes('update_')) {
        console.log(`📝 Notary: ${event.actor} → ${event.command} (${event.key})`);
    }
...
function clearLedger() {
    ...
    ledger.length = 0;
    console.warn(`⚠️  Notary ledger cleared (${count} entries removed)`);
    return count;
}
