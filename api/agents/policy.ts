 ...
 */
const DEFAULT_AUTHORITY_MATRIX: Record<AgentType, AuthorityLevel> = {
 ...
 */
const AUTHORITY_CONSTRAINTS: Record<
  AgentType,
  Record<AuthorityLevel, Record<string, any>>
> = {
  moderation_sentinel: {
    read: {},
    suggest: {
...
export async function getAuthority(agentType: AgentType): Promise<AgentPolicy> {
  ...
  const authorityLevel: AuthorityLevel = flag?.enabled
    ? (flag.metadata?.authority_level as AuthorityLevel) || DEFAULT_AUTHORITY_MATRIX[agentType]
