...
FOR UPDATE USING (
  ...
  id = auth.uid() AND
  role = (SELECT role FROM users WHERE id = auth.uid()) AND -- Can't change own role
  is_admin = (SELECT is_admin FROM users WHERE id = auth.uid()) -- Can't change own admin status
);
...
DO $$
BEGIN
  RAISE NOTICE '✅ Owner/Admin security setup complete!';
