...
exports.handleSubscriptionCanceled = handleSubscriptionCanceled;
exports.grantTrial = grantTrial;
exports.grantPromo = grantPromo;
...
async function getUserTier(userId) {
    ...
    return entitlement.entitlement_value?.tier || 'free';
}
/**
 * Check if user has entitlement
 */
async function hasEntitlement(userId, entitlementKey) {
 ...
 */
async function getEntitlement(userId, entitlementKey) {
    const tier = await getUserTier(userId);
    const { data: definition } = await supabase
        .from('entitlement_definitions')
        .select('*')
 ...
 */
async function syncStripeSubscription(userId, stripeSubscriptionId) {
    try {
 ...
 */
async function handleSubscriptionCanceled(userId, stripeSubscriptionId) {
 ...
 */
async function grantTrial(userId, durationDays = 7) {
 ...
 */
async function grantPromo(userId, entitlementKey, value, durationDays) {
 ...
 */
async function enforceLimit(userId, metricKey) {
...
async function getEntitlementSummary(userId) {
    ...
    for (const def of definitions || []) {
        const userEntitlement = entitlements?.find((e) => e.entitlement_key === def.entitlement_key);
