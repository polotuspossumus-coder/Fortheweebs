 ...
 * Multi-provider: Printful (apparel), Mixam (comics), DriveThruCards (trading cards)
 */
...
const { PRINT_SERVICES, calculatePricing, routeToProvider } = require('../utils/printServiceConfig');
...
/**
 * Get Available Print Services & Products
 * GET /api/merch/print-services
 */
router.get('/print-services', (req, res) => {
    res.json({
        services: PRINT_SERVICES,
        message: 'Available print-on-demand services',
        setup: {
            printful: 'Add PRINTFUL_API_KEY to .env',
            mixam: 'Add MIXAM_API_KEY to .env',
            drivethrucards: 'Add DRIVETHRUCARDS_API_KEY to .env'
        }
    });
});
 ...
 * POST /api/merch/calculate-pricing
 */
router.post('/calculate-pricing', (req, res) => {
    try {
        const { productType, salePrice, printCost } = req.body;
        
        if (!productType || !salePrice || !printCost) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        
        const provider = routeToProvider(productType);
        if (!provider) {
        ...
        res.json({
            ...
            category,
            ...pricing,
            breakdown: `Sale: $${pricing.salePrice} - Print: $${pricing.printCost} = $${pricing.totalProfit} profit (Creator: $${pricing.creatorEarnings}, Platform: $${pricing.platformFee})`
        });
