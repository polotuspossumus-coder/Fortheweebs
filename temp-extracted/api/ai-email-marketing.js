...
router.post('/generate-subject', async (req, res) => {
    try {
        const { topic, goal = 'open_rate' } = req.body;
        // goal: 'open_rate', 'click_rate', 'conversions'
...
router.post('/create-workflow', async (req, res) => {
    try {
        const { workflowType, trigger } = req.body;
...
router.get('/analytics', async (req, res) => {
    try {
        const { campaignId, timeRange = '30days' } = req.query;
