...
export default function MobileGovernanceVote({ proposal }) {
  ...
  return (
    <div className="p-4 max-w-sm mx-auto" aria-label="Governance vote">
      ...
      <button className="w-full bg-green-600 text-white py-2 rounded" onClick={() => vote('yes')} aria-label="Vote yes">Vote Yes</button>
      <button className="mt-2 w-full bg-red-600 text-white py-2 rounded" onClick={() => vote('no')} aria-label="Vote no">Vote No</button>
    </div>
  );
}
