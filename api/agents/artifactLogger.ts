...
export type AuthorityLevel = 'read' | 'suggest' | 'act' | 'enforce';
 ...
 */
export async function logArtifact(entry: ArtifactEntry): Promise<string> {
 ...
 */
export async function sealBundle(bundle: ArtifactBundle): Promise<string> {
  const { data, error } = await supabase
 ...
 */
export async function* streamArtifacts(filters?: {
  agentType?: AgentType;
  since?: Date;
}) {
...
export async function getActivitySummary(hours: number = 24) {
  ...
  if (error) {
    ...
    const summary = {
      ...
      byAuthority: {} as Record<AuthorityLevel, number>,
      topActions: [] as Array<{ action: string; count: number }>,
    };

    artifacts.forEach((artifact) => {
      summary.byAgent[artifact.agent_type as AgentType] =
        (summary.byAgent[artifact.agent_type as AgentType] || 0) + 1;

      if (artifact.authority_level) {
        summary.byAuthority[artifact.authority_level as AuthorityLevel] =
          (summary.byAuthority[artifact.authority_level as AuthorityLevel] || 0) + 1;
      }
    });
