...
DO $$
BEGIN
  -- Check if bug_reports table exists first
...
-- RLS Policies for ai_conversations (with proper casting)
...
-- RLS Policies for suggestions (with proper casting)
...
CREATE POLICY "Only owner can view notifications"
  ...
  FOR SELECT
  USING (
    auth.email() = 'owner@fortheweebs.com' -- ⚠️ REPLACE WITH YOUR EMAIL
  );
