-- ===================================
-- FORTHEWEEBS SECURITY SYSTEM MIGRATIONS
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='users' AND column_name='chargeback_count') THEN
        ALTER TABLE users ADD COLUMN chargeback_count INTEGER DEFAULT 0;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='users' AND column_name='feature_usage') THEN
        ALTER TABLE users ADD COLUMN feature_usage JSONB DEFAULT '{}';
    END IF;

    -- Activity count (for fraud detection)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_name='users' AND column_name='activity_count') THEN
...
-- Admin-only access policies (you'll need to adjust based on your admin role setup)
-- These are examples - modify based on your auth system

CREATE POLICY "Admin can view fraud logs" ON fraud_analysis_logs
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM users
...
CREATE POLICY "Admin can view payment attempts" ON payment_attempts
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM users
      WHERE users.id = auth.uid()
...
CREATE POLICY "Admin can view admin alerts" ON admin_alerts
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users
      WHERE users.id = auth.uid()
...
-- Schedule cleanup (you can run this manually or set up a cron job)
-- Example: SELECT cleanup_old_logs();

-- ===================================
-- INITIAL DATA
-- ===================================

-- Insert a test admin alert (optional)
-- INSERT INTO admin_alerts (type, severity, details, requires_action)
-- VALUES ('SYSTEM_INITIALIZED', 'INFO', '{"message": "Security system initialized"}', FALSE);
...
-- COMPLETED!
-- ===================================
-- All security tables have been created
